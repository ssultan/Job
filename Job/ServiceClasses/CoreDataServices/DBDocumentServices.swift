...
class DBDocumentServices: CoreDataBusiness {
    ...
    class func getAllDocumentsThoseAreDeleted() -> [DocumentModel] {
        let managedObjectContext = CoreDataManager.sharedInstance.managedObjectContext
        let predicate = NSPredicate(format: "isSent = %@  AND isPhotoDeleted = %@ AND jobInstance.photoAckReceived = %@ AND (jobInstance.manifest.user.userName = %@ OR answer.jobInstance.manifest.user.userName = %@)", NSNumber(value: true), NSNumber(value: true), NSNumber(value: true), AppInfo.sharedInstance.username ?? "", AppInfo.sharedInstance.username ?? "")
